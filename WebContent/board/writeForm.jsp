...
<script type="text/javascript">
	$(function() {
		...
		$("#title").keyup(function() { //키 누를때
			//alert("title - keyup");
			var title = $(this).val(); // 제목입력칸
			if (title.indexOf(" ") == 0) { // 맨앞에 공백 입력되면
				$(this).val($.trim(title)); // title 의 값을 trim()해서 덮어씌운다
			}
		});

		$("#writeForm").submit(function() {
			$("title").val($.trim(("#title").val())); //title값 trim해서 덮어씌운다
			$("#title").val(title);
			if (title.length < 4) {
				$("#title").focus();
				return false;
...
<body>
	<div class="container">
	   ...
	   -->
			...
			<!-- 비밀번호 입력 -->
			<div class="form-group">
				<label for="pw">비밀번호</label> <input type="text" class="form-control"
